...
Here is an example from the `{ohwhaley}` package so you can see what field corresponds to what outcome:
...
Have some utility functions that you don't want to share in your Reference section? No probs! Pop these using a section with the `title` field set to `internal`. Note that there is **NO `-`** in front of the **contents** field
